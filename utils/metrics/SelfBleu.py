...
class SelfBleu(Metrics):
    def __init__(self, original_text='', transfer_text='', gram=3, name='SelfBleu', portion=1):
        super().__init__()
        self.name = name
        self.original_data = original_text
        self.transfer_data = transfer_text
        self.gram = gram
    ...
    def get_bleu(self):
        ngram = self.gram
        bleu = list()
        reference = self.get_text(self.original_data)
        test = self.get_text(self.transfer_data)
        weight = tuple((1. / ngram for _ in range(ngram)))

        for i in range(len(test)):
            refer =reference[i]
            hypo = test[i]
            bleu.append(self.calc_bleu(refer, hypo, weight))
