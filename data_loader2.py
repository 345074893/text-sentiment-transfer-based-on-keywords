...
class RealDataLoader():
    def __init__(self, batch_size, seq_length):
        ...
        fsent_word = open('data/yelp/english_stopword', 'r')
        fpos_word = open('data/sentiment_dic/positive-words.txt', 'r')
        fneg_word = open('data/sentiment_dic/negative-words.txt', 'r')
        english_punctuations = [',', '.', ':', ';', '?', '(', ')', '[', ']', '&', '!', '*', '@', '#', '$', '%', '...',
                                '-', "\'s", 'n\'t', '``']
        # self.stopword_list = [word.strip('\n') for word in fsent_word] + english_punctuations
        self.stopword_list = [word.strip('\n') for word in fsent_word]
        self.pos_word_list = [word.strip('\n') for word in fpos_word]
        self.neg_word_list = [word.strip('\n') for word in fneg_word]
        self.sent_word_list = self.pos_word_list + self.neg_word_list
    ...
    def makeData(self, srcFile, label):
        ...
        # keep_word_property = ['NN', 'NNS', 'VB', 'VBD', 'VBG', 'VBN', 'VBP', 'VBZ', 'PRP', 'PRP$']
        # keep_word_property = ['NN', 'NNS', 'VB', 'VBD', 'VBG', 'VBN', 'VBP', 'VBZ']
        ...
        for i in srcF:
            ...
            keyword = [word for word in split_text if word not in self.stopword_list]
            keyword = [self.style_word if word in self.sent_word_list else word for word in keyword]
        ...
        Dataset = {"text": original_text, "text_keyword": text_keyword, "text_length": original_length,
                   ...
                   "text_keyword_id": text_keyword_id, "labels": original_label}
...
# oracle_loader = RealDataLoader(128, 17)
# _, index2word = oracle_loader.Vocabulary('data/yelp/yelp_15/vocab')
# oracle_loader.create_batches(pos_file='data/yelp/yelp_15/sentiment.dev1.text', neg_file='data/yelp/yelp_15/sentiment.dev0.text')
